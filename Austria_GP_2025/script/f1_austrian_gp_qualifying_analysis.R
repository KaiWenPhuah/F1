# Formula 1 Austrian GP 2025 Qualifying Analysis
...
aus_quali1 <- aus_quali %>% 
  select(-c(X, Time, DriverNumber, PitOutTime, PitInTime, 
            Sector1SessionTime, Sector2SessionTime, Sector3SessionTime, 
            LapStartDate, LapStartTime, Position, Deleted, FastF1Generated, 
            laptime_sum_sectortimes)) %>% 
  drop_na(LapTime_in_seconds)

View(aus_quali1)

# 3. Best Qualifying Lap of Each Driver 
...
# 4. Fastest 5 Drivers in Each Sector
...
# 5. Driver Lap Progression by Team 
...
aus_quali1 %>% 
  ...
  geom_line() +
  geom_point() +
  labs(title = "Lap Progression Between Verstappen and Tsunoda",
       subtitle = "Laps From Q1 to Q3",
       x = "Lap Number",
       y = "Total Lap Time (Seconds)") +
  theme_minimal() +
...
aus_quali1 %>% 
  ...
  geom_line() +
  geom_point() +
  labs(title = "Lap Progression Between Leclerc and Hamilton",
       subtitle = "Laps From Q1 to Q3",
       x = "Lap Number",
       y = "Total Lap Time (Seconds)") +
  theme_minimal() +
...
aus_quali1 %>% 
  ...
  geom_line() +
  geom_point() +
  labs(title = "Lap Progression Between Russell and Antonelli",
       subtitle = "Laps From Q1 to Q3",
       x = "Lap Number",
       y = "Total Lap Time (Seconds)") +
  theme_minimal() +
...
# 7. Improvement Between First and Best Lap 
...
ggplot(time_diff) +
  ...
  labs(title = "Most Improved Performance Between First and Best Lap",
...
# 8. Average Pace for Fresh vs Used Tyres 
...
ggplot(tyre_pace_diff) +
  ...
  geom_text(aes(x = reorder(Driver, pace_diff), y = pace_diff, 
                label = round(pace_diff, 2)), 
            hjust = -0.5, size = 3) +
  labs(title = "Time Gained on Fresh Tyres vs Used",
       subtitle = "Measured in Seconds",
       x = "Drivers",
       y = "Time (Seconds)") +
  coord_flip() +
...
ggplot(ideal_laps) +
  ...
  labs(title = "Gap to Ideal Laptime",
